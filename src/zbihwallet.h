...
#ifndef Bithost_ZBIHWALLET_H
#define Bithost_ZBIHWALLET_H
...
public:
    CzBIHWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZBIH(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZBIH(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Bithost_ZBIHWALLET_H

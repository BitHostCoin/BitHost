...
CzBIHWallet::CzBIHWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbih seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBIHSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBIHSeed_deprecated()) {
                    LogPrintf("%s: Updated zBIH seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbih seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbih wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBIHWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBIHCount(nCountLastUsed);
    else if (!walletdb.ReadZBIHCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBIHWallet::Lock()
{
    seedMaster = 0;
}

void CzBIHWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBIHWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBIH(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBIHWallet::LoadMintPoolFromDB()
{
...
void CzBIHWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBIHWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBIHWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbihTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBIHWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBIH(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbihTracker which also adds to database
    pwalletMain->zbihTracker->Add(dMint, true);
...
void CzBIHWallet::SeedToZBIH(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBIHWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBIHWallet::GenerateDeterministicZBIH(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBIHWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBIHWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{

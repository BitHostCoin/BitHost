...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZBihStake(spend));
    } else {
        ...
        CBihStake* bihInput = new CBihStake();
        bihInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(bihInput);
    }

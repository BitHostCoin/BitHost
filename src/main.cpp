...
#if defined(NDEBUG)
#error "Bithost cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ubih) are considered zero fee (for relaying and mining)
...
{
	//Check to see if the zBIH is properly signed
	if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
		if (!spend.HasValidSignature())
			return error("%s: V2 zBIH spend does not have a valid signature", __func__);

		libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
		if (tx.IsCoinStake())
			expectedType = libzerocoin::SpendType::STAKE;
		if (spend.getSpendType() != expectedType) {
			return error("%s: trying to spend zBIH without the correct spend type. txid=%s", __func__,
				tx.GetHash().GetHex());
		}
	}
	...
	if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
		return error("%s : zBIH spend with serial %s is already in block %d\n", __func__,
	...
	if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
		...
		return error("%s : zBIH spend with serial %s from tx %s is not in valid range\n", __func__,
...

	{
		...
		if (tx.IsZerocoinSpend()) {
			...
			if (IsTransactionInChain(tx.GetHash(), nHeightTx))
				return state.Invalid(error("AcceptToMemoryPool : zBIH spend tx %s already in block %d",
			...
			for (const CTxIn& txIn : tx.vin) {
				...
				if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
					return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
						tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zbih");
			}
		}
		else {
			LOCK(pool.cs);
			...
			// Check that zBIH mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight <= Params().LAST_POW_BLOCK() && nHeight >= 1) {
	...
	else if (nHeight <= 36000 && nHeight > 18000) {
		nSubsidy = 12 * COIN;
	}
	else if (nHeight <= 50000 && nHeight > 36000) {
		nSubsidy = 15 * COIN;
	}
	...
	else if (nHeight <= 124000 && nHeight > 100000) {
		nSubsidy = 30 * COIN;
	}
	...
	else if (nHeight <= 250000 && nHeight > 225000) { // Start of new reward structure
		nSubsidy = 32 * COIN;
		...
		nSubsidy = 16.38 * COIN;
	} else if (nHeight <= 500000 && nHeight > 400000) { // Left off here, reward structure given not clear
		nSubsidy = 24 * COIN;
	}
...
int nStartTreasuryBlock = 220000;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	for (int i = block.vtx.size() - 1; i >= 0; i--) {
		...
		* note we only undo zerocoin databasing in the following statement, value to and from Bithost
...
{
	RenameThread("bithost-scriptch");
	scriptcheckqueue.Thread();
}

void RecalculateZBIHMinted()
{
...
void RecalculateZBIHSpent()
{
	...
	while (true) {
		...
		//Remove spends from zBIH supply
...
bool RecalculateBIHSupply(int nHeightStart)
{
...
bool UpdateZBIHSupply(const CBlock& block, CBlockIndex* pindex)
{
	std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
	...
	if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
		...
		RecalculateBIHSupply(Params().Zerocoin_StartHeight());
	}

	//Track zBIH money supply in the block index
	if (!UpdateZBIHSupply(block, pindex))
		return state.DoS(100, error("%s: Failed to calculate new zBIH supply for block=%s height=%d", __func__,
	...
	//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zBihSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
	...
	// If turned on AutoZeromint will automatically convert BIH to zBIH
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
	...
	for (const CTransaction& tx : block.vtx) {
		...
		// double check that there are no double spent zBIH spends in this block
		if (tx.IsZerocoinSpend()) {
			for (const CTxIn& txIn : tx.vin) {
				if (txIn.scriptSig.IsZerocoinSpend()) {
					libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
					if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
						return state.DoS(100, error("%s : Double spending of zBIH serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
	if (nHeight < Params().Zerocoin_Block_V2_Start())
		return error("%s: zBIH stake block is less than allowed start height", __func__);

	if (CZBihStake* zBIH = dynamic_cast<CZBihStake*>(stake)) {
		CBlockIndex* pindexFrom = zBIH->GetIndexFrom();
		if (!pindexFrom)
			return error("%s: failed to get index associated with zBIH stake checksum", __func__);

		if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
			return error("%s: zBIH stake does not have required confirmation depth", __func__);
		...
		uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zBIH->GetValue()));
		if (nChecksum200 != zBIH->GetChecksum())
			return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zBIH->GetChecksum(), nChecksum200);
	}
	else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (block.IsProofOfStake()) {
		...
		if (stake->IsZBIH() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
			return state.DoS(100, error("%s: staked zBIH fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
	...
	if (nMints || nSpends)
		LogPrintf("%s : block contains %d zBIH mints and %d zBIH spends\n", __func__, nMints, nSpends);

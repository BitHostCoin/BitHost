...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 BIH at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBIH send
enum ZerocoinSpendStatus {
    ZBIH_SPEND_OKAY = 0,                            // No error
    ZBIH_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBIH_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBIH_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBIH_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBIH_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBIH_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBIH_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBIH_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBIH_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBIH_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBIH_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBIH_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBIH_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBIH_SPENT_USED_ZBIH = 14,                      // Coin has already been spend
    ZBIH_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZBIH_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZBihBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZBIHOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzbihAuto) const;
    ...
    */
    ...
    CzBIHWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzBIHTracker> zbihTracker;
    ...
    void setZWallet(CzBIHWallet* zwallet)
    {
        zwalletMain = zwallet;
        zbihTracker = std::unique_ptr<CzBIHTracker>(new CzBIHTracker(strWalletFile));
    }

    CzBIHWallet* getZWallet() { return zwalletMain; }
    ...
    void setZBihAutoBackups(bool fEnabled)
    {
    ...
    /** zBIH reset */
    boost::signals2::signal<void()> NotifyzBIHReset;

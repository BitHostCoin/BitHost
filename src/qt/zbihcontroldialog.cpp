...
#include "ui_zbihcontroldialog.h"
...
std::set<std::string> ZBihControlDialog::setSelectedMints;
std::set<CMintMeta> ZBihControlDialog::setMints;

ZBihControlDialog::ZBihControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZBihControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZBihControlDialog::~ZBihControlDialog()
{
    delete ui;
}

void ZBihControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBihControlDialog::updateList()
{
...
void ZBihControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBihControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZBih_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBihControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZBihControlDialog::GetSelectedMints()
{
...
void ZBihControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);

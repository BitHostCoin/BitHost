...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BIH);
    ...
    if (!settings.contains("nAnonymizeBithostAmount"))
        settings.setValue("nAnonymizeBithostAmount", 1000);

    nAnonymizeBithostAmount = settings.value("nAnonymizeBithostAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBithostAmount"))
        SoftSetArg("-anonymizebithostamount", settings.value("nAnonymizeBithostAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bithost.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBithostAmount:
            return QVariant(nAnonymizeBithostAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBithostAmount:
            nAnonymizeBithostAmount = value.toInt();
            settings.setValue("nAnonymizeBithostAmount", nAnonymizeBithostAmount);
            emit anonymizeBithostAmountChanged(nAnonymizeBithostAmount);
            break;
        case CoinControlFeatures:

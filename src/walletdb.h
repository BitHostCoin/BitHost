...
public:
    ...
    bool WriteZBIHSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBIHSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBIHSeed_deprecated(uint256& seed);
    bool EraseZBIHSeed();
    bool EraseZBIHSeed_deprecated();

    bool WriteZBIHCount(const uint32_t& nCount);
    bool ReadZBIHCount(uint32_t& nCount);
